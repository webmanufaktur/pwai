#!/bin/bash
# Core maintainer functions

main() {
    # Ensure required directories exist
    mkdir -p "$TASKS_DIR/active" "$TASKS_DIR/backlog" "$TASKS_DIR/completed"
...
new_feature() {
    ...
    echo "2. Add tasks with: $0 add task $feature_id 'Task description'"
}
...
show_status() {
    ...
    find "$FEATURES_DIR" -maxdepth 1 -name "*.md" ! -name "README.md" | sort | while read -r f; do
        local status=$(grep -m1 "Status:" "$f" 2>/dev/null | cut -d: -f2- | xargs || echo "Unknown")
